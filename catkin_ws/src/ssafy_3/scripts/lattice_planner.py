...
class latticePlanner:
    ...
    def latticePlanner(self, ref_path, vehicle_status):
        ...
        if len(ref_path.poses) > look_distance:
            ...
            ref_path.poses[look_distance * 2].pose.position.x, ref_path.poses[look_distance * 2].pose.position.y)
            ...
            for i in range(look_distance * 2, add_point_size):
                if i + 1 < len(ref_path.poses):
                    ...
                    tmp_t = np.array([[cos(tmp_theta), -sin(tmp_theta), tmp_translation[0]],
                                      [sin(tmp_theta), cos(tmp_theta), tmp_translation[1]], [0, 0, 1]])
            ...
            '''
            for i in range(len(out_path)):
                globals()['lattice_pub_{}'.format(i + 1)] = rospy.Publisher('/lattice_path_{}'.format(i + 1), Path,
                                                                            queue_size=1)
                globals()['lattice_pub_{}'.format(i + 1)].publish(out_path[i])
